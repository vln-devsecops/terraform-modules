...
variable "kms_key_arn" {
  description = "Optional existing KMS key ARN. When omitted the module creates a dedicated CMK."
  type        = string
  default     = null
}
...
variable "source_bucket_arn" {
  description = "ARN of the deployment source bucket. Must be in us-east-1 for Lambda@Edge."
  type        = string
}

variable "source_bucket_id" {
  description = "Name or ID of the deployment source bucket."
  type        = string
}

variable "source_object_key" {
  description = "Optional S3 object key for the deployment archive. Defaults to app_name-function_name.zip."
  type        = string
  default     = null
}
