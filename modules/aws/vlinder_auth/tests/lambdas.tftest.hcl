...
run "rotate_secret_role_can_only_write_the_four_rotatable_secrets" {
  command = plan
  ...
  assert {
    condition = (
      ...
      strcontains(aws_iam_policy.rotate_secret[0].policy, one(aws_secretsmanager_secret.admin_api_csrf_secret[*].arn))
    )
    error_message = "rotate_secret's PutSecretValue grant should be scoped to exactly the four secrets it rotates, not a wildcard."
  }
}
...
run "rotation_schedules_target_rotate_secret_with_the_right_input" {
  ...
  assert {
    condition = (
      ...
      one(aws_scheduler_schedule.rotate_admin_api_csrf_secret[*].schedule_expression) == "rate(30 days)"
    )
    error_message = "All four rotation schedules should fire every 30 days -- an adopter should never need to redeploy just to rotate a key."
  }
  ...
  assert {
    condition = (
      jsondecode(one(aws_scheduler_schedule.rotate_admin_api_csrf_secret[*].target)[0].input).secretId == one(aws_secretsmanager_secret.admin_api_csrf_secret[*].id) &&
      jsondecode(one(aws_scheduler_schedule.rotate_admin_api_csrf_secret[*].target)[0].input).passwordLength == 64
    )
    error_message = "The admin-api-csrf-secret schedule should target that secret specifically, with its own 64-length password (no fixed-byte-count requirement, unlike the one-time-token/refresh-token keys)."
  }
  ...
  assert {
    condition = (
      ...
      one(aws_scheduler_schedule.rotate_admin_api_csrf_secret[*].target)[0].arn == aws_lambda_function.rotate_secret[0].arn
    )
    error_message = "All four schedules should target the same rotate_secret Lambda -- it isn't hardcoded to one secret."
  }
}
...
run "rotation_infra_is_omitted_for_the_identity_only_profile" {
  ...
  assert {
    condition = (
      ...
      length(aws_scheduler_schedule.rotate_admin_api_csrf_secret) == 0
    )
