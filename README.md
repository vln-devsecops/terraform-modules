...
| `modules/aws/vlinder_auth` | Self-provisioning branded auth component: identifier-first own-UI signup/login (no hosted UI; Cognito behind a first-party `/api/v1/auth` surface), DynamoDB-backed RBAC (role/privilege separation, single- or multi-tenant, login-vs-sudo role activation), published Lambda triggers, and a bundled admin panel + API — minimal input, no consumer-managed Lambda ARNs or tables |
...
| `modules/aws/http_api` | API Gateway HTTP API (v2) with Lambda integrations, per-route JWT, `AWS_IAM`, or Lambda (`CUSTOM`) authorization, and optional custom domain |
| `modules/aws/http_api_authorizer` | Reusable Lambda REQUEST authorizer for `http_api`'s `CUSTOM` routes: rejects requests missing a shared origin-verify header, optionally also verifying a JWT |
...
| `modules/aws/static_site` | CloudFront + S3 static site with ACM, Route 53, SPA fallback, WAF, access logging, response headers policy, and an opt-in origin-response Lambda@Edge association |
